# -*- coding: utf-8 -*-
"""
Double deep Q-network implementation with chainer and rlglue
...
class ddqn_agent(Agent):  # RL-glue Process
    lastAction = Action()
    policyFrozen = False
    ...
    def agent_step(self, reward, observation):
        ...
        if self.policyFrozen is False:  # Learning ON/OFF
            if self.DDQN.initial_exploration < self.time:
                self.epsilon -= 1.0/10**6
                if self.epsilon < 0.1:
                    self.epsilon = 0.1
                eps = self.epsilon
            else:  # Initial Exploation Phase
                print "Initial Exploration : %d/%d steps" % (self.time, self.DDQN.initial_exploration)
                eps = 1.0
        else:  # Evaluation
        ...
        returnAction = Action()
        action, Q_now = self.DDQN.e_greedy(state_, eps)
        returnAction.intArray = [action]

        # Learning Phase
        if self.policyFrozen is False:  # Learning ON/OFF
            self.DDQN.stockExperience(self.time, self.last_state, self.lastAction.intArray[0], reward, self.state, False)
            self.DDQN.experienceReplay(self.time)

        # Target model update
        if self.DDQN.initial_exploration < self.time and np.mod(self.time, self.DDQN.target_model_update_freq) == 0:
            print "########### MODEL UPDATED ######################"
            self.DDQN.target_model_update()

        # Simple text based visualization
        print ' Time Step %d /   ACTION  %d  /   REWARD %.1f   / EPSILON  %.6f  /   Q_max  %3f' % (self.time, self.DDQN.action_to_index(action), np.sign(reward), eps, np.max(Q_now.get()))
    ...
    def agent_end(self, reward):  # Episode Terminated
        ...
        # Learning Phase
        if self.policyFrozen is False:  # Learning ON/OFF
            self.DDQN.stockExperience(self.time, self.last_state, self.lastAction.intArray[0], reward, self.last_state, True)
            self.DDQN.experienceReplay(self.time)
...
if __name__ == "__main__":
    AgentLoader.loadAgent(ddqn_agent())
